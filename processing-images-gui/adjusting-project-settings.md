...
* ✅ **Kalibracija refleksije**: Omogućeno (zahtijeva slike meta MAPIR)
* ✅ **Debayer metoda**: Visoka kvaliteta (brže)
* ✅ **Format za izvoz**: TIFF (16-bitni)

Jednostavno uvezite svoje slike i počnite s obradom s ovim zadanim postavkama.

***

## Pregled postavki projekta
...
Kontrolira kako Chloros identificira ciljeve kalibracije na vašim slikama.

**Ključne postavke:*** **Minimalno područje uzorka za kalibraciju**: prag veličine za otkrivanje cilja (zadano: 25 piksela)
...
* Smanjenje ako se ciljevi ne otkrivaju
...
**Ključne postavke:*** **Korekcija vinjete**: kompenzira zatamnjenje leće na rubovima ✅ Preporučeno
* **Kalibracija refleksije**: normalizira vrijednosti pomoću ciljeva kalibracije ✅ Preporučeno
* **Debayerova metoda**: Algoritam za pretvaranje RAW u 3-kanalni višespektralni
* **Minimalni interval rekalibracije**: Vrijeme između korištenja ciljeva kalibracije (0 = koristiti sve)**Napredne postavke:*** **Pomak vremenske zone senzora svjetla**: Za PPK sinkronizaciju vremena (zadano: 0)
* **Primijeni PPK ispravke**: koristi GPS/podatke o ekspoziciji iz .daq datoteka
* **Igla ekspozicije 1/2**: Dodjeljuje kamere iglama ekspozicije za postavke s dvije kamere
...
**Popularni indeksi:*** **NDVI**: Opće zdravlje vegetacije (najčešće)
* **NDRE**: Rano otkrivanje stresa s RedEdge
* **GNDVI**: osjetljiv na koncentraciju klorofila
* **OSAVI**: Dobro radi s vidljivom zemljom
...
**Dostupni formati:*** **TIFF (16-bit)**: Preporučeno za GIS i znanstvene analize (0-65,535 raspon)
...
* **JPG (8-bitni)**: Najmanje datoteke, kompresija s gubitkom (0-255 raspon)***

## Spremanje i učitavanje postavki
...
2. Dođite do odjeljka **"Spremi predložak projekta"** na dnu
3. Unesite opisni naziv predloška (npr. "Survey3N\_RGN\_Agriculture")
4. Pritisnite ikonu za spremanje

**Prednosti:**

* Primijenite identične postavke na više projekata
* Podijelite konfiguracije s članovima tima
...
3. Odaberite svoj spremljeni predložak
...
## Postavljanje PPK (naknadno obrađene kinematike).
...
2. U postavkama projekta omogućite potvrdni okvir **"Primijeni PPK ispravke"**

3. Po potrebi postavite**"Pomak vremenske zone senzora svjetla"** (zadano: 0 za UTC)
4. Dodijelite kamere iglama ekspozicije:
   * **Jedna kamera**: Automatski se dodjeljuje Pin 1
   * **Dvostruke kamere**: Ručno dodijelite svakoj kameri ispravan pin**Dodjela igle za ekspoziciju:*** **Oznaka ekspozicije 1**: Odaberite model kamere s padajućeg izbornika
...
2. U svakoj sesiji koristite dosljednu postavku cilja kalibracije
...
* Razmotrite CLI ili API za serijsku automatizaciju
...
1. **Označite ciljne slike kalibracije** - pogledajte [Odabir ciljnih slika](choosing-target-images.md)
